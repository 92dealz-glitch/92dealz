    ...
    <div className="bg-white p-6 rounded-2xl border border-zinc-200 shadow-sm mt-8">
      <div className="flex items-center justify-between mb-6">
        ...
        <button
          ...
          className="flex items-center gap-2 bg-blue-600 text-white px-4 py-2 rounded-lg font-black uppercase text-[10px] tracking-widest hover:bg-blue-700 transition-all shadow-md active:scale-95"
        >
          <UserPlus size={18} />
          Add CSR Staff
        </button>
                ...
                <tr key={s.id} className="group hover:bg-blue-50/20 transition-colors">
                  ...
                  <td className="px-4 py-4 w-1/3">
                    <div className="text-zinc-600 text-xs italic bg-zinc-50 p-2 rounded-lg border border-zinc-100 group-hover:bg-white transition-colors relative pr-8">
                       ...
                       <button onClick={() => setNoteModal({ show: true, id: s.id, note: s.note || "" })} className="absolute right-2 top-2 text-zinc-400 hover:text-blue-500">
                          <Edit3 size={14} />
                       </button>
                    </div>
                  </td>
           ...
           <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl relative overflow-hidden animate-in zoom-in-95 duration-200">
             ...
             <form onSubmit={handleSubmit} className="p-6 space-y-4">
               ...
               <div className="space-y-1">
                 ...
                 <input type="text" name="name" required value={formData.name} onChange={handleChange} className="w-full rounded-xl border border-zinc-200 px-4 py-3 text-sm focus:ring-2 focus:ring-blue-300 outline-none transition font-bold" />
               </div>
               <div className="space-y-1">
               ...
               <div className="space-y-1">
                 ...
                 <div className="relative">
                   <input type={showPassword ? "text" : "password"} name="password" required value={formData.password} onChange={handleChange} className="w-full rounded-xl border border-zinc-200 px-4 py-3 text-sm focus:ring-2 focus:ring-blue-300 outline-none transition pr-11 font-bold" />
               ...
               <button type="submit" disabled={loading} className="w-full bg-blue-600 text-white font-black py-4 rounded-xl hover:bg-blue-700 transition-all shadow-lg shadow-blue-100 disabled:opacity-50 flex items-center justify-center gap-2 mt-2 uppercase text-xs tracking-widest">
        ...
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl relative overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="p-6 bg-blue-50 border-b border-blue-100 flex items-center gap-3">
              <div className="w-8 h-8 bg-blue-100 text-blue-600 rounded-lg flex items-center justify-center"><Edit3 size={18} /></div>
              <h3 className="font-black uppercase tracking-tight text-blue-900">Admin Note</h3>
            </div>
            <div className="p-6 space-y-4">
              <div>
                <textarea 
                  ...
                  rows={5} 
                  className="w-full border border-zinc-200 rounded-xl p-4 text-sm font-medium outline-none focus:ring-2 focus:ring-blue-200 resize-none transition-all" />
              </div>
              <div className="flex gap-3 pt-2">
                ...
                <button disabled={actionLoading !== null} onClick={handleNoteUpdate} className="flex-[1.5] py-3.5 bg-blue-600 text-white rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-blue-700 shadow-lg shadow-blue-100 disabled:opacity-50 transition-all flex items-center justify-center gap-2">
