...
export default function HotDeals() {
  ...
  font-extrabold text-black text-center"
        >
          Hot Deals <span className="text-[#708238]">Today!</span>
        </h3>

        <div className="ml-auto hidden md:flex">
          <a
            href="#"
            className="text-sm font-medium text-gray-700 hover:text-[#708238] transition"
          >
            See more
          </a>
        </div>
      ...
      <div className="relative">
        ...
        <button
          ...
          onClick={scrollLeft}
          className="hidden lg:flex absolute left-0 top-1/2 -translate-y-1/2 -translate-x-6 z-20 bg-[#708238] w-14 h-14 rounded-full items-center justify-center text-white shadow-lg hover:bg-[#5E6E2F] transition"
        >
          <svg
            className="w-6 h-6"
            fill="none"
        ...
        >
          {list.map((item) => (
            <div
              key={item.id}
              className="relative shrink-0 w-[160px] xs:w-[180px] sm:w-[200px] md:w-[220px] lg:w-[250px] bg-white rounded-[1.2rem] sm:rounded-[1.5rem] border-2 border-[#C7A27C] p-2.5 shadow-sm h-full flex flex-col snap-start group"
            >
              {/* FAVORITE BUTTON */}
              {(() => {
              ...
              <Link href={`/product/${item.id}`} className="block h-full">
                ...
                <div className="relative aspect-square rounded-xl overflow-hidden bg-gray-50">
                  ...
                  {item.isVerified && (
                    <div className="absolute top-2 left-2 z-10 bg-white rounded-full p-0.5 shadow-sm">
                       <svg viewBox="0 0 24 24" fill="none" className="w-4 h-4 text-[#C7A27C]" xmlns="http://www.w3.org/2000/svg">
                ...
                <div className="mt-2.5 flex flex-col flex-1 px-0.5">
                  <p className="text-[#708238] font-black text-base sm:text-lg leading-none">
                    {item.price}
                  </p>
                  ...
                  <div className="mt-0.5 flex items-center gap-0.5 text-amber-400 leading-none">
        ...
        <button
          ...
          onClick={scrollRight}
          className="hidden lg:flex absolute right-0 top-1/2 -translate-y-1/2 translate-x-6 z-20 bg-[#708238] w-14 h-14 rounded-full items-center justify-center text-white shadow-lg hover:bg-[#5E6E2F] transition"
        >
          <svg
            className="w-6 h-6"
            fill="none"
