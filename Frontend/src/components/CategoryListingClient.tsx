    ...
    <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
      ...
      <div className="md:hidden flex items-center justify-between mb-2 col-span-full">
          <button
              ...
              className="flex items-center gap-2 px-4 py-2 bg-white border-2 border-[#E9E0D4] rounded-xl font-bold text-[#708238] shadow-sm active:scale-95 transition-all w-full justify-center"
          >
        ...
        >
          ...
          <FilterSection label="Price Range" open={openPrice} onToggle={() => setOpenPrice((v) => !v)}>
            ...
            <div className="mt-6 pt-6 border-t border-[#E9E0D4]/30">
              ...
              {(customMin || customMax) && (
                <button
                  onClick={() => { setCustomMin(""); setCustomMax(""); setPage(1); }}
                  className="mt-3 text-xs font-bold text-[#708238] hover:text-[#5E6E2F] underline"
                >
                  Reset budget
                </button>
              )}
          ...
          {isMobileFilterOpen && (
              <div className="p-5 mt-4">
                  <button
                      onClick={() => setIsMobileFilterOpen(false)}
                      className="w-full py-4 bg-[#708238] text-white font-bold rounded-2xl shadow-lg shadow-[#C7A27C] active:scale-95 transition-all text-lg"
                  >
                      Apply Filters
                  </button>
              </div>
      ...
      <section className="md:col-span-9">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-8 gap-4 px-1">
          <div className="text-base text-gray-900 font-medium">
            Showing <span className="font-bold text-[#708238]">{filtered.length}</span> verified results
          </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {paged.length === 0 ? (
            <div className="p-12 bg-white border-2 border-dashed border-[#E9E0D4]/40 rounded-3xl col-span-full text-center">
        ...
        {totalPages > 1 && (
          <div className="mt-12 flex items-center justify-center gap-3">
            <button
              ...
              className="w-10 h-10 flex items-center justify-center border-2 rounded-xl text-sm bg-white disabled:opacity-30 border-[#E9E0D4] hover:border-[#708238] transition-colors"
            >
